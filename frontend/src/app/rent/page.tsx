      ...
      <div className="max-w-7xl mx-auto pb-12">
        ...
        <div className="mb-8 flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-bold text-[#1E293B]">Rent a Book</h1>
              <p className="text-[#64748B] mt-0.5">Borrow physical copies from our library</p>
            </div>
          </div>
          <div className="bg-[#EEF2FF] border border-[#C7D2FE] rounded-xl px-4 py-3 text-sm text-[#4338CA] hidden md:block">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <h3 className="text-lg font-semibold text-[#1E293B] mb-2">No books available for rent</h3>
            <p className="text-[#64748B] text-sm">Check back later for available physical copies.</p>
          </div>
        ) : (
          <div className="bg-white rounded-xl shadow-sm border border-[#E2E8F0] overflow-hidden">
            <div className="bg-[#F8FAFC] border-b border-[#E2E8F0] px-6 py-3 grid grid-cols-12 text-xs font-semibold text-[#64748B] uppercase tracking-wider">
              ...
              {books.map((book) => (
                ...
                >
                  ...
                  <div className="col-span-5 flex items-center gap-4">
                    <div className="w-12 h-16 bg-gradient-to-br from-[#EEF2FF] to-[#E0E7FF] rounded-lg overflow-hidden shrink-0 shadow-sm border border-[#E2E8F0]">
                      {book.coverUrl ? (
                    ...
                    <div className="min-w-0">
                      <h3 className="font-semibold text-sm text-[#1E293B] truncate">{book.title}</h3>
                  ...
                  {/* Category */}
                  <div className="col-span-2 text-sm text-[#64748B]">{book.category}</div>
                  ...
                  <div className="col-span-2 text-center">
                    <span className={`text-sm font-semibold ${book.hardCopyCount > 0 ? "text-emerald-600" : "text-red-500"}`}>
                      {book.hardCopyCount} left
                    </span>
                  </div>

                  {/* Action */}
                  <div className="col-span-2 flex items-center justify-end gap-2">
                    <span className="text-xs font-bold text-[#3B4FE8]">Rs.{book.rentPrice}/day</span>
                    <Link
                      href={`/books/${book._id}`}
                      onClick={() => handleRent(book)}
                      className={`text-xs font-semibold px-3 py-1.5 rounded-lg transition-colors ${
                        book.hardCopyCount > 0 && book.status === "in-shelf"
                          ? "bg-[#3B4FE8] text-white hover:bg-[#2d3dd4]"
